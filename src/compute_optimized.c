...
int convolve(matrix_t *a_matrix, matrix_t *b_matrix, matrix_t **output_matrix) {
  ...
  uint32_t cut_off = (row_diff + 1) - ((row_diff + 1) % num_threads);
  if (leftover != 0) {       //as long as there is even one row left to go... 
    #pragma omp parallel num_threads(leftover)
    {
      int thread_num = omp_get_thread_num();
      int num_threads = omp_get_num_threads();
      uint32_t work = 1           //might not divide perfectly so need to do manual work afterword
      uint32_t start = (work * thread_num) + cut_off;
      uint32_t finish = start + work;
